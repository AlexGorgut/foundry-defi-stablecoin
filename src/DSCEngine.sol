 ...
 * - Exogenous Collateral(застава): ETH & BTC
 * - Dollar Peg: USD
 * - Algorithmically Stable
 *
 * Our DSC system should always be "overcollateralized". At no point should the value of the collateral
 *  be less than the value of the DSC tokens.
 *
 * It si similar to DAI if DAI had no governance, no oracles, and no liquidations and was only backed by WETH and WBTC.
 *  @notice This is the contract that governs the DSC system. It is the only contract that can mint and burn DSC tokens.
...
contract DSCEngine is ReentrancyGuard {
    ...
    */
    ...
    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view returns (uint256) {
        ...
        // 1 ETH = $1000
        // The returned value from CL will be 1000 * 1e8 (1e8 is the precision)
        return ((usdAmountInWei * PRECISION) / (uint256(price) * ADDITIONAL_FEED_PRECISION));
    }

    function getAccountCollateralValue(address user) public view returns (uint256 totalCollateralValueInUsd) {
        // loop through all collateral token, get the amount they have deposited, and map it to
